...
function removeG() {
  return d3.select('.graph').select('svg').selectAll('g').remove();
}
/**
...
exports.filterCrimeType = filterCrimeType;
exports.filterSeasons = filterSeasons;
exports.filterTimePeriod = filterTimePeriod;
exports.fillMissingData = fillMissingData;
exports.getSeason = getSeason;
exports.filterYearlyDataByCrimeType = filterYearlyDataByCrimeType;
exports.filterCrimeByMomentOfDay = filterCrimeByMomentOfDay;
...
function filterYears(data) {
  ...
  data.forEach(function (line) {
    ...
    var yearIndex = -1;
    filteredData.forEach(function (elem, index) {
...
function filterCrimeType(data) {
  var filteredData = [];
  data.forEach(function (line) {
    var typeIndex = -1;
    filteredData.forEach(function (typeData, index) {
...
function filterSeasons(crimeTypeData, seasons) {
  var filteredData = [];
  crimeTypeData.forEach(function (typeData) {
    var _filteredData$push;

    filteredData.push((_filteredData$push = {
      type: typeData.type
    }, _defineProperty(_filteredData$push, seasons.Winter, 0), _defineProperty(_filteredData$push, seasons.Spring, 0), _defineProperty(_filteredData$push, seasons.Summer, 0), _defineProperty(_filteredData$push, seasons.Autumn, 0), _filteredData$push));
    typeData.data.forEach(function (line) {
...
function filterTimePeriod(crimeTypeData) {
  var filteredData = [];
  crimeTypeData.forEach(function (typeData) {
    typeData.data.forEach(function (line) {
...
function filterYearlyDataByCrimeType(data) {
  var filteredData = [];
...
function filterCrimeByMomentOfDay(data) {
  var filteredData = [];
  data.forEach(function (line) {
    var momentIndex = -1;
    filteredData.forEach(function (elem, index) {
  ...
  filteredData.forEach(function (moment) {
    var filteredCrimes = [];
    moment.types.forEach(function (crime) {
      var crimeIndex = -1;
      filteredCrimes.forEach(function (elem, index) {
...
function getCategories(viz1Data) {
  var crimeTypes = [];
  viz1Data.forEach(function (yearlyData) {
    yearlyData.crimes.forEach(function (count, category) {
 ...
 */
function setColorScaleDomain(colorScale, categories) {
...
function updateXScale(xScale, viz1Data, width) {
  xScale.domain(viz1Data.map(function (x) {
    return x.year;
  }).sort()).range([0, width]);
}
/**
...
function updateYScale(yScale, viz1Data, height) {
  var totals = [];
  viz1Data.forEach(function (yearlyData) {
    var yearlyTotal = 0;
    yearlyData.crimes.forEach(function (count) {
      yearlyTotal += count;
    });
...
function drawYAxis(yScale, width) {
  d3.select('.y.axis').call(d3.axisLeft(yScale).tickFormat(function (x) {
    return x.toString();
  }));
}
/**
...
function createGroups(viz1Data) {
  d3.select('#graph-g').selectAll('.group').data(viz1Data).join('g').attr('class', 'group');
}
/**
...
function drawBars(y, x, color) {
  var lastY = 0;
  var count = 0;
  d3.select('#graph-g').selectAll('.group').selectAll('rect').data(function (d) {
    var crimes = [];
    d.crimes.forEach(function (value, key) {
      crimes.push({
        year: d.year,
        type: key,
        count: value
...
},{}],"scripts/viz2.js":[function(require,module,exports) {
"use strict";
 ...
 * @param {Function} range A utilitary funtion that could be useful to generate a list of numbers in a range
 */
function updateXScale(xScale, viz2Data, width) {
  xScale.domain(viz2Data.map(function (x) {
    return x.type;
  }).sort()).range([0, width]);
}
/**
...
function updateYScale(yScale, viz2Data, height, seasons) {
  var totals = [];
  viz2Data.forEach(function (line) {
...
function drawBars(y, x, color, data, SEASONS) {
  var stackedData = d3.stack().keys([SEASONS.Winter, SEASONS.Spring, SEASONS.Summer, SEASONS.Autumn])(data);
  d3.select('#graph-g').selectAll('.group').selectAll('g').data(stackedData).join('g').classed('series', true).attr('fill', function (d) {
    return color(d.key);
 ...
 * @param {Function} range A utilitary funtion that could be useful to generate a list of numbers in a range
 */
function updateXScale(xScale, timePeriods, width) {
...
function updateYScale(yScale, crimeTypes, height) {
...
function drawXAxis(xScale) {
  d3.select('.x.axis').call(d3.axisTop(xScale).tickFormat(function (x) {
    return x;
  }));
}
/**
...
function updateRects(xScale, yScale, colorScale, viz3Data) {
  d3.select('#graph-g').selectAll().data(viz3Data).join('rect').attr('x', function (d) {
...
function drawLegend(colorScale, g, width) {
  ...
  var legend = _d3SvgLegend.default.legendColor().scale(colorScale).title('Legend').shape('path', d3.symbol().type(d3.symbolCircle).size(250)());
...
function rectSelected(element, xScale, yScale) {
  d3.select(element.parentNode).append('text').attr('text-anchor', 'middle').attr('x', function (d) {
    return xScale(d.Plantation_Year);
  }).attr('y', function (d) {
    return yScale(d.Arrond_Nom);
  }).attr('transform', 'translate(' + element.attributes.width.value / 2 + ',' + element.attributes.height.value / 2 + ')').text(function (d) {
    return d.Comptes.toString();
  }).style('dominant-baseline', 'central').style('fill', function (d) {
    return d.Comptes > 1000 ? 'white' : 'black';
...
function selectTicks(name, year) {
  d3.select('.y.axis').selectAll('.tick').filter(function (d) {
    return d == name;
  }).select('text').attr('font-weight', 'bold');
...
var viz1 = _interopRequireWildcard(require("./scripts/viz1.js"));

var viz2 = _interopRequireWildcard(require("./scripts/viz2.js"));
...
(function (d3) {
  ...
  d3.csv('./interventionscitoyendo.csv', d3.autoType).then(function (data) {
    var timePeriods = preproc.getTimePeriod(data); //viz1 preprocess
    ...
    function setClickHandlerViz1() {
      d3.select('#viz-button1').on('click', function () {
        buildViz1();
      });
    }
    /**
    ...
    function setClickHandlerViz2() {
      d3.select('#viz-button2').on('click', function () {
        buildViz2();
      });
    }
    /**
    ...
    function setClickHandlerViz3() {
      d3.select('#viz-button3').on('click', function () {
        buildViz3();
      });
    }
    /**
    ...
    function setClickHandlerViz4() {
      d3.select('#viz-button4').on('click', function () {
        buildViz4();
      });
    }
    /**
    ...
    function setClickHandlerViz5() {
      d3.select('#viz-button5').on('click', function () {
        build();
      });
    }
    /**
    ...
    function buildViz1() {
      ...
      helper.appendAxes(g);
      viz1.updateXScale(xBandScale, viz1Data, graphSize.width - padding, util.range); //helper.updateXSubgroupScale(xSubgroupScale, categories, xBandScale) todo: sert a rien?
