...
exports.getViz2Data = getViz2Data;
exports.getViz3Data = getViz3Data;
exports.getViz4Data = getViz4Data;
...
function filterCrimeTypeName(data) {
...
function filterDataByDateAndCrimeType(data) {
  var filteredData = [];
...
function getViz2Data(data, SEASONS) {
  ...
  data.forEach(function (crime) {
    ...
    if (index == -1) {
      ...
      viz2Data.push((_viz2Data$push = {
        type: crime.type
      }, _defineProperty(_viz2Data$push, SEASONS.Winter, 0), _defineProperty(_viz2Data$push, SEASONS.Spring, 0), _defineProperty(_viz2Data$push, SEASONS.Summer, 0), _defineProperty(_viz2Data$push, SEASONS.Autumn, 0), _viz2Data$push));
...
function getViz3Data(data, TYPES, TIME_PERIODS) {
  var viz3Data = [];
  data.forEach(function (crime) {
...
function updateXScale(xScale, data, width) {
  xScale.domain(data.map(function (x) {
    return x.year;
  }).sort()).range([0, width]);
}
/**
...
function updateYScale(yScale, viz1Data, height) {
  ...
  viz1Data.forEach(function (yearlyData) {
    var yearlyTotal = yearlyData['Vol de véhicule à moteur'] + yearlyData['Méfait'] + yearlyData['Vols qualifiés'] + yearlyData['Introduction'] + yearlyData['Vol dans / sur véhicule à moteur'] + yearlyData['Infractions entrainant la mort'];
...
function drawBars(y, x, color, viz1Data) {
  var stackedData = d3.stack().keys(['Vol de véhicule à moteur', 'Méfait', 'Vols qualifiés', 'Introduction', 'Vol dans / sur véhicule à moteur', 'Infractions entrainant la mort'])(viz1Data);
...
function createGroups(viz2Data) {
  d3.select('#graph-g').selectAll('.group').data(viz2Data).join('g').attr('class', 'group');
}
/**
 * Draws the bars inside the groups
 *
...
function drawLegend(colorScale, g, width) {
  ...
  var legend = _d3SvgLegend.default.legendColor().scale(colorScale).title('Légende').shape('path', d3.symbol().type(d3.symbolCircle).size(250)());
...
(function (d3) {
  ...
  var yBandScale = d3.scaleBand().padding(0.2);
  ...
  d3.csv('./interventionscitoyendo.csv', d3.autoType).then(function (data) {
    ...
    var TYPES = preproc.getTypes(data);
    ...
    */
    ...
    function buildViz2() {
      ...
      var filteredViz2Data = preproc.keepChosenSeasons(viz2Data, chosenSeasons, SEASONS);
